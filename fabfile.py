...
from aiohttp.web_routedef import put
...
def backup():
    ...
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick '
            'awesome > %s' % (db_user, db_password, f))
